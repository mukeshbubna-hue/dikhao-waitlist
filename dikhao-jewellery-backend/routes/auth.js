...
// POST /api/auth/verify-otp  { mobile, otp, store_name?, owner_name?, state?, city? }
router.post('/verify-otp', async (req, res) => {
  const { mobile, otp, store_name, owner_name, state, city } = req.body;
  ...
  if (!store) {
    ...
    const insert = await supabase.from('stores').insert({
      mobile, store_name, owner_name, state, city,
    }).select('*').single();
